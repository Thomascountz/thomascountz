---
layout: post
title: "Ruby ORM VI: Metaprogramming Column-backed Query Methods"
...
Ruby gives us access to metaprogramming, which I like to think of as just programming with meta-data. In this specific example, we use the `#define_method()` method, which does exactly what it sounds like.
...
Like before, we can write this function in the `Modelable::ClassMethods` module, and it will be available to our client class, `Book`.
