---
layout: post
title: "Ruby ORM II: Getting SQLite Talking to Ruby"
...
The next few lines create the columns of our table. `id`, `title`, `author`, and `isbn`, each hold the data belonging to each book. The interesting one to note is `id`, which is declared as `INTEGER PRIMARY KEY`
...
From the client's perspective, they're only dealing with a PORO, _plain old Ruby object_. Without debating the pros and cons of obfuscating database interactions, I think our `Book` class does a pretty good job!
