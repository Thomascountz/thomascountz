---
layout: post
title: "Ruby ORM V: Choosing Mixins over Inheritance"
tags: [project, series__peel, ruby, databases]
---
...
There are endless debates about composition v. inheritance, and, from my perspective, they all mostly lean towards composition, but what about using mix-ins versus using single inheritance?
...
As mentioned above, `include` will add an entity in the method look-up path. So for our `book#title`, `book#author`, and `book#isbn`, this sounds like the kind of functionality we want. The users of our ORM shouldn't have to define these methods, they should be provided to them. If the method is missing from their class, we can define them in our module and place it next in line.
...
First, we’ll want a way to read our database columns in order to know what accessor methods we need to build. In SQLite, we can use `PRAGMA`
